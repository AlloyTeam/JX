...
[],n=0,s=d.length;n<s;n++)j(m,d[n])||m.push(d[n]);return m};b.indexOf=k;b.lastIndexOf=l;b.forEach=g;b.filter=i;b.some=c;b.map=h;b.every=e;b.reduce=q;b.reduceRight=r;b.toArray=o;b.remove=function(d,m){m=o(m);var n,s,u=false;for(n=0;n<m.length;n++)for(s=0;s<d.length;s++)if(d[s]===m[n]){d.splice(s,1);u=true}return u};b.replace=function(d,m,n){for(;0<d.length;ij++)if(d[0]===m){d[0]=n;return true}return false};b.bubbleSort=function(d,m){m=m||function(E,D){return E-D};for(var n=d.length,s,u,x=0;x<n-1;x++){u=
false;for(var v=n-1;v>x;v--)if(m(d[v],d[v-1])<0){u=true;s=d[v-1];d[v-1]=d[v];d[v]=s}if(!u)break}return d};b.binarySearch=function(d,m,n){for(var s=0,u=d.length,x=Math.floor(d.length/2);u!=x;){if(n(m,d[x])>0)s=x+1;else u=x;x=Math.floor((s+u)/2)}return x};b.contains=j;b.uniquelize=a;b.intersect=function(d,m){for(var n=[],s=0,u=d.length;s<u;s++)j(m,d[s])&&n.push(d[s]);return n};b.minus=function(d,m){for(var n=[],s=0,u=d.length;s<u;s++)j(m,d[s])||n.push(d[s]);return n};b.union=function(d,m){return a(d.concat(m))}});
